...
void config_load_from(Config *c, const wchar_t *subkey)
{
    ...
    if (reg_get_f(k, L"clearcoat", &f))       c->clearcoat = f;
    if (reg_get_f(k, L"anisotropy", &f))      c->anisotropy = f;
    ...
    c->clearcoat = clampf(c->clearcoat, 0.0f, 1.0f);
    c->anisotropy = clampf(c->anisotropy, 0.0f, 1.0f);
...
void config_save_to(const Config *c, const wchar_t *subkey)
{
    ...
    set_f(k, L"clearcoat", c->clearcoat);
    set_f(k, L"anisotropy", c->anisotropy);
